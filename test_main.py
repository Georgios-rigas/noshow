...
from pydantic import BaseModel, Field # Keep Field if used, otherwise can remove
...
S3_BUCKET_NAME = 'noshow-model'
S3_PREPROCESSOR_KEY = 'models/preprocessor_retrained.joblib.gz'
S3_CLASSIFIER_KEY = 'models/classifier_retrained.joblib.gz'
...
def load_components():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            print(f"Attempting to download preprocessor from s3://{S3_BUCKET_NAME}/{S3_PREPROCESSOR_KEY}")
            s3_client = boto3.client('s3') # Assumes credentials are configured in the environment
            ...
            print(f"S3 Load Error (Preprocessor): AWS credentials not found. Ensure the FastAPI environment has credentials (e.g., IAM role).")
        ...
        try:
            print(f"Attempting to download classifier from s3://{S3_BUCKET_NAME}/{S3_CLASSIFIER_KEY}")
            # s3_client should already be initialized if preprocessor download was attempted
            if 's3_client' not in locals(): # Initialize if not already (e.g. if preprocessor step failed early)
        ...
        if not preprocessor_loaded_s3 or not classifier_loaded_s3:
            ...
            # Ensure they are None if loading failed
        ...
        # are automatically cleaned up when tmpdir goes out of scope.
    ...
    try:
        ...
        for item in snowflake_payload.data:
            if not isinstance(item, tuple) or len(item) != 2:
                ...
                # Consider how to handle errors for individual rows in a batch
                # For now, we'll let Pydantic validation on SnowflakeRequest handle this mostly
                continue 
            ...
            try:
                features_dict = actual_features.model_dump() 
                input_df = pd.DataFrame([features_dict], columns=feature_names_in_order)
                
                # print(f"Processing row index: {row_index}") # Debugging
        ...
        if snowflake_format_response:
            ...
            # If only one row was processed (e.g. MAX_BATCH_ROWS=1 from Snowflake, or direct non-batch call)
            # return the simpler format if requested.
            if len(response_data_for_snowflake) == 1:
                return {"prediction": response_data_for_snowflake[0][1]}
            else: # Should not happen if snowflake_format_response is False and batch > 1
